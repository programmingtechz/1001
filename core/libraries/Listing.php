...
class Listing
{
    ...
    {                                   
        ...
        $this->_CI->encryption->initialize(array('driver' => 'openssl'));                        
    ...
    public function render($callback, $criteria = array(), $variables=array())
    {
        ...
        $data['per_page_options']  = array_combine($this->_get_per_page_options(), $this->_get_per_page_options());
		...
		$data['namespace'] = $this->_CI->encryption->encrypt($this->_CI->namespace."|{$variables['model']}|{$variables['method']}");
    ...
    public function get_result($callback, Criteria $criteria = null, $namespace_model_method = FALSE)
    {
    	...
    	{
    		$temp = explode( '|', $this->_CI->encryption->decrypt($namespace_model_method) );
    		if(count($temp) == 3)
    		{
    			$namespace 	= $temp[0];
    			$model 		= $temp[1];
    ...
    function get_listings($model = null, $method = null,$args = array())
    {
        ...
        $criteria['custom_query_args'] = $args;

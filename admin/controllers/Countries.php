...
class Countries extends Admin_Controller
{
  ...
  public function index()
  {
    ...
    //ACTION BUTTONS
    $str = '<a href="'.site_url('countries/add/{id}').'" class="btn btn btn-padding yellow table-action"><i class="fa fa-edit edit"></i></a><a href="javascript:void(0);" data-original-title="Remove" data-toggle="tooltip" data-placement="top" class="table-action btn-padding btn red" onclick="delete_record(\'countries/delete/{id}\',this);"><i class="fa fa-trash-o trash"></i></a>';    
    $this->listing->initialize(array('listing_action' => $str));

    $listing = $this->listing->get_listings('countries_model', 'listing');
    
    $this->data['btn'] = "<a href=".site_url('countries/add')." class='btn btn-primary'>Add New <i class='fa fa-plus'></i></a>";
    ...
    $this->layout->view('/pages/countries/index');
  }
  ...
  function bulk_actions()
  {
     ...
     switch( $action ){
        ...
        case 'delete':
            ...
            if( $bulk_all ){
                //$this->countries_model->delete($delete_ids);
                $this->countries_model->delete(array('1'=>'1'));
        ...
        case 'export':
            ...
            $query = $this->countries_model->get_where($selected_ids,'name, code');
     ...
     try
      {
          ...
          if ($this->form_validation->run())
          {
              ...
              $whr = array("code" => $ins_data['code']);
              
              if($edit_id){
                $whr['id!='] = $edit_id;
              }
              
              if( $this->countries_model->get_where( $whr )->num_rows() ){
                $msg = 'Country Code Already Exist';
                $flash_msg_type = 'error_msg';
              }
              else if($edit_id)
              {
                $this->countries_model->update( array("id" => $edit_id), $ins_data  );
                $msg = 'Record updated successfully';
              }
              else
              { 
                $country_id    = $this->countries_model->insert(  $ins_data,  "countries");
      ...
      if($edit_id)
      {
        $edit_data =  $this->countries_model->get_where(array("id" => $edit_id))->row_array();;
      }
      ...
      $this->layout->view('pages/countries/add');
   }
  ...
  function delete($del_id)
  {
    ...
    $access_data = $this->countries_model->get_where(array("id"=>$del_id),'id')->row_array();     
  
    $output=array();
  
    if(count($access_data) > 0)
    {
      $this->countries_model->delete(array("id"=>$del_id));

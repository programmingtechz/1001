...
class Cities extends Admin_Controller
{
  ...
  public function index()
  {
    ...
    //ACTION BUTTONS
    $str = '<a href="'.site_url('cities/add/{id}').'" class="btn btn btn-padding yellow table-action"><i class="fa fa-edit edit"></i></a><a href="javascript:void(0);" data-original-title="Remove" data-toggle="tooltip" data-placement="top" class="table-action btn-padding btn red" onclick="delete_record(\'cities/delete/{id}\',this);"><i class="fa fa-trash-o trash"></i></a>';    
    $this->listing->initialize(array('listing_action' => $str));

    $listing = $this->listing->get_listings('cities_model', 'listing');
    
    $this->data['btn'] = "<a href=".site_url('cities/add')." class='btn btn-primary'>Add New <i class='fa fa-plus'></i></a>";
    ...
    $this->layout->view('/pages/cities/index');
  }
  ...
  function bulk_actions()
  {
     ...
     switch( $action ){
        ...
        case 'delete':
            ...
            if( $bulk_all ){
                //$this->cities_model->empty_table();
                $this->cities_model->delete(array('1'=>'1'));
                
            }else {
                $delete_ids = array('id' => $selected_ids);
                $this->cities_model->delete($delete_ids);
            }
        ...
        case 'export':
            ...
            $query = $this->cities_model->get_where($selected_ids,'name, code');
     ...
     try
      {
          ...
          if ($this->form_validation->run())
          {
              ...
              $whr = array("state_id" => $ins_data['state_id'],"code" => $ins_data['code'],"country_id" => $ins_data['country_id']);
              
              if($edit_id){
                $whr['id!='] = $edit_id;
              }
              
              if( $this->cities_model->get_where( $whr )->num_rows() ){
                $msg = 'City Code Already Exist for Selected Country & State';
                $flash_msg_type = 'error_msg';
              }
              else if($edit_id)
              {
                $this->cities_model->update( array("id" => $edit_id), $ins_data  );
                $msg = 'Record updated successfully';
              }
              else
              { 
                $city_id    = $this->cities_model->insert(  $ins_data,  "cities");
      ...
      if($edit_id)
      {
        $edit_data =  $this->cities_model->get_where(array("id" => $edit_id))->row_array();;
      }
      ...
      $this->layout->view('pages/cities/add');
   }
  ...
  function delete($del_id)
  {
    ...
    $access_data = $this->cities_model->get_where(array("id"=>$del_id),'id')->row_array();     
  
    $output=array();
  
    if(count($access_data) > 0)
    {
      $this->cities_model->delete(array("id"=>$del_id));

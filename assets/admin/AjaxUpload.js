...
ss.SimpleUpload = function( options ) {
  ...
  this._opts = {
    ...
    videoExtensions: [ 'avi', 'mkv', 'mov', '3g2' ,'3gp' ,'3gpp' ,'asf' ,'dat' ,'divx' ,'dv' ,'f4v' ,'flv' ,'m2ts' ,'m4v' ,'mod' ,'mp4' ,'mpe' ,'mpeg' ,'mpeg4' ,'mpg' ,'mts' ,'nsv' ,'ogm' ,'ogv' ,'qt' ,'tod' ,'ts' ,'vob' ,'wmv'],
    ...
    endUpload: function() {},
    startUpload: function(total_files) {return true;}
  };
...
ss.SimpleUpload.prototype = {
  ...
  */
  _createInput: function() {
    ...
    if ( XhrOk && this._opts.webkitdirectory ) {
    ...
    ss.addEvent( this._input, 'change', function() {
      ...
      self._opts.total_file_count = self._input.files.length;
      
      if( self._opts.total_file_count > self._opts.maxUploads ){
            EC.UI.alert( 'Maximum uploads limited to '+self._opts.maxUploads);
            return;
      }
     
      if( false === self._opts.startUpload.call(self, self._opts.total_file_count)){
     ...
     $(this._input).off('dropUpload').on('dropUpload', function(e,file_data) {
        var filename,
          ext,
          total,
          i;
          ...
          self._opts.total_file_count=0;
          self._active =0;
          self._opts.is_customized = true;
                       
        self._opts.total_file_count = file_data.files.length;
        ...
        if ( (!self._input || self._input.value === '') && !file_data.files ) {
        return;
       }

        filename = ss.getFilename( file_data.files[0].name );
        ext = ss.getExt( filename );
        ...
        total = file_data.files.length;

        for ( i = 0; i < total; i++ ) {
          self._queue.push( file_data.files[i] );
        }
    ...
    {
      //custom code
      if(self._input.name =='uploadfile'){
        //$('#post_manager .post-text .btn_attach').html( div );
        $( self._btns[0] ).html( div );
      }
      else if( self._input.name.indexOf('bulk_upload_file_') != -1) {
        var class_id = self._input.name.split(/[_]+/).pop();
        $('body').find('.bl_img_'+class_id.toString()).html( div );
      }
    }, 1);
  },
  ...
  */
  _checkFile: function( filename, ext, size ) {
    ...
    if ( size && this._opts.imageExtensions.indexOf( ext ) != -1 &&
        this._opts.maxSize !== false &&
        size > this._opts.maxSize )
    {
      this.removeCurrent();
      this.log( filename + ' exceeds ' + this._opts.maxSize + 'K limit' );
      this._opts.onSizeError.call( this, filename, size, this._opts.maxSize,ext );
      return false;
    }
   else if ( size && this._opts.videoExtensions.indexOf( ext ) != -1 &&
        this._opts.videoMaxSize !== false &&
        size > this._opts.videoMaxSize )
    {
      this.removeCurrent();
      this.log( filename + ' exceeds ' + this._opts.videoMaxSize + 'K limit' );
      this._opts.onSizeError.call( this, filename, size, this._opts.videoMaxSize,ext );
      return false;
    }
  ...
  */
  submit: function() {
    ...
    if ( false === this._opts.onSubmit.call( this, filename, ext, this._file,this ) ) {
        
        if( !this._opts.is_customized )
            this.removeCurrent();
            
      return;
    }
    
     if( this._opts.is_customized ) return;
